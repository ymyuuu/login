...
(async () => {
  ...
  const CT8 = fs.readFileSync('accounts.json', 'utf-8');
  const accounts = JSON.parse(CT8);
  ...
  for (const account of accounts) {
    ...
    let url = `https://panel.ct8.pl/login/?next=/`;
    ...
    try {
      ...
      // 用户之间添加随机延时
      const delay = Math.floor(Math.random() * 8000) + 5000; // 随机延时1秒到8秒之间
